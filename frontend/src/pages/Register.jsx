  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (data.session?.access_token) {
        // Project has email confirmation OFF — Supabase returned a session
        // straight away, so log the new user straight into the app.
        await setSession(data.session.access_token)
        navigate('/dashboard', { replace: true })
      } else {
        // Project has email confirmation ON — no session yet.
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12">
        <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-5">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Set up operator access for this deployment.</p>
          </div>
          ...
          {info && (
            <div className="rounded-lg border border-emerald-900 bg-emerald-950/50 px-3.5 py-2.5 text-sm text-emerald-300">
              {info}
            </div>
          )}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-slate-400" htmlFor="confirm-password">Confirm password</label>
            <input
              id="confirm-password"
              type="password"
              required
